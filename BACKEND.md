...
| `{action:'board.update', id, data:{…}}` | 내 글 수정 | 글쓴이만 (`FORBIDDEN`). 보낸 항목만 바뀐다(부분 수정). `kind:'done'` → 모집 완료, `kind:'team'` → 다시 모집 중 |
...
느린 원인은 **Apps Script 한 번 왕복이 기본 1~2초**라는 데 있습니다. 코드를 빠르게 만드는 것보다 **왕복 횟수를 줄이는 것**이 효과가 큽니다.

| 손본 곳 | 전 | 후 |
|---|---|---|
| 글 쓰기·수정·삭제 | 쓰기 1번 + 목록 다시 읽기 1번 = **2왕복** | 쓰기 응답에 목록을 같이 담아 **1왕복** |
| 토큰 검증 | 요청마다 구글 `tokeninfo` 에 물음 (0.3~0.6초) | 5분간 `CacheService` 에 담아 **재사용** |
| 서버 버전 확인(`probe`) | 화면을 옮길 때마다 물음 | 탭이 살아 있는 동안 `sessionStorage` 에 **기억** |
| 첫 요청 | 컨테이너를 띄우느라 1~2초 더 | 첫 화면이 그려지면 **미리 한 번 깨워 둠** |
| 신청서 조회 | `findMine_` 이 한 실행에서 두 번 시트를 읽음 | 한 번만 읽고 **재사용** |
| 로그인 직후 | 안 보는 화면의 요청이 먼저 나갈 수 있었음 | **보고 있는 화면부터** 요청 |

> `api: 4` 는 "쓰기 응답에 목록이 들어 있다"는 뜻입니다. 옛 서버(`api: 3`)에 붙어도 화면은 예전처럼 목록을 다시 읽으므로 그냥 동작합니다 — 느릴 뿐입니다.
